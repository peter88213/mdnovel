...
class PluginManager(tk.Toplevel):
    ...
    def __init__(self, size, view, controller, **kw):
        ...
        super().__init__(**kw)
        self.title(f'{_("Installed plugins")} - mdnovel @release')
        self.geometry(size)
        self.grab_set()
        self.focus()
        ...
        columns = 'Module', 'Version', 'mdnovel API', 'Description'
        ...
        self._moduleCollection.column('mdnovel API', width=100, minwidth=100, stretch=False)
        self._moduleCollection.heading('mdnovel API', text=_('mdnovel API'), anchor='w')
