...
class PlotLineJson(BasicElementNotesJson):

    def import_data(self, element, json):
        super().import_data(element, json)
        element.shortName = json.get('ShortName', None)
        element.sections = json.get('Sections', [])

    def export_data(self, element, json):
        json = super().export_data(element, json)
        if element.shortName:
            json['ShortName'] = element.shortName
        if element.sections:
            json['Sections'] = element.sections
        return json
